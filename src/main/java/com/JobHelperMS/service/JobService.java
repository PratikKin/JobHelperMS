...
public class JobService {
    ...
    public Job createJob(JobRequest request) {
        ...
        Job job = new Job(); // Generate sequential job ID
        ...
        job.setId("JOB_" + nextId); // Set job fields
        ...
        job.setHrId(request.getRequestInfo().getId());
        ...
        job.setLastUpdated(request.getJob().getPostedDate()); // Resolve skills by ID from DB
        List<Skill> skills = request.getJob().getSkills().stream() .map(skillRef -> skillRepository.findById(skillRef.getId()) .orElseThrow(() -> new RuntimeException("Skill not found: " + skillRef.getId()))) .toList();
